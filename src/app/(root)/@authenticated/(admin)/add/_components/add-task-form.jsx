...
export const AddTaskForm = ({ isModal}) => {
    ...
    const form = useForm ({
     ...
     async function onSubmit(values) {
            ...
            if (!base.title || !base.ownerId) {
              setErrorMessage("Title and assigned user are required.")
              setSubmitted(false)
              return
            }
                  ...
                  <Command>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {users.map((user) => (
                          <CommandItem
                          value={`${user.displayName} ${user.email}`.toLowerCase()}
                            key={user.uid}
                            onSelect={() => {
       ...
       <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                {reoccuringType === "none" && 'välj hur ofta uppgifter ska upprepas. Väljer du "ingen" så är en engångsuppgift'}
                {reoccuringType === "multiple" && 'Välj flera dagar som du vill ha uppgiften på.'}
                {reoccuringType === "range" && 'Välj ett start och slutdatum för uppgiften kommer att upprepas varje dag mellan dessa datum.'}
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
